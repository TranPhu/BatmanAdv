...
{
//   printk(KERN_ALERT "[PhuTran] Go to batadv_interface_tx ..........\n");
...
send:
	...
	if (do_bcast)
	{
	  printk(KERN_ALERT "[PhuTran] Send packet broadcast\n");
	...
	{
	  printk(KERN_ALERT "[PhuTran] Send packet unicast(%pM): %pM\n",soft_iface->dev_addr,ethhdr->h_source);
		...
		if (dhcp_rcp == BATADV_DHCP_TO_SERVER)
		{
			...
			printk(KERN_ALERT "[PhuTran] OK: batadv_send_skb_via_gw %i\n",ret);
		} 
		else if (mcast_single_orig)
		{
			...
			printk(KERN_ALERT "[PhuTran] OK: batadv_send_skb_unicast %i\n",ret);
		} 
		else 
		{
			...
			ret = batadv_send_skb_via_tt(bat_priv, skb, dst_hint,
						     vid);
			printk(KERN_ALERT "[PhuTran] OK: batadv_send_skb_via_tt %i\n",ret);
		}
...
{
  printk(KERN_ALERT "[PhuTran] Receive Packet(%pM): (%pM)\n",soft_iface->dev_addr, skb->dev->dev_addr);
				   ...
				   struct net_device *slave_dev)
{
	...
	if (!hard_iface || hard_iface->soft_iface != NULL)
		goto out;
     printk(KERN_ALERT "[PhuTran] Add net_dev: %s and %s\n",dev->name,slave_dev->name);
	ret = batadv_hardif_enable_interface(hard_iface, dev->name);
// 	printk(KERN_ALERT "Send OGMs packet: 7-0\n");
...
static void batadv_softif_init_early(struct net_device *dev)
{
	...
	ether_setup(dev);
	//dev->dev_addr = "74:2f:68:af:0b:6d";
	...
	printk(KERN_ALERT "[PhuTran] Init device:(%pM)\n",dev->dev_addr);
}

struct net_device *batadv_softif_create(const char *name)
{
  printk(KERN_ALERT "[PhuTran] Adding interface: %s\n",name);
	struct net_device *soft_iface;
	int ret;
