...
{
//   printk(KERN_ALERT "[PhuTran] Go to batadv_sysfs_add_meshif funtion\n");
...
{
   printk(KERN_ALERT "[PhuTran] Go to batadv_sysfs_add_vlan : %s\n",dev->name);
...
static ssize_t batadv_store_mesh_iface(struct kobject *kobj,
				       ...
				       size_t count)
{
  printk(KERN_ALERT "[PhuTran] Store_mesh_interface\n");
	struct net_device *net_dev = batadv_kobj_to_netdev(kobj);
	struct batadv_hard_iface *hard_iface;
	int status_tmp = -1;
	int ret = count;
...
// 	printk(KERN_ALERT "Send OGMs packet: 7-1\n");
...
{
  printk(KERN_ALERT "[PhuTran] Go to batadv_sysfs_add_hardif\n");
	...
	*hardif_obj = kobject_create_and_add(BATADV_SYSFS_IF_BAT_SUBDIR,
	...
	for (bat_attr = batadv_batman_attrs; *bat_attr; ++bat_attr) {
      ...
      printk(KERN_ALERT "[PhuTran] Go to batadv_sysfs_add_hardif (2nd)\n");
	return 0;

rem_attr:
	for (bat_attr = batadv_batman_attrs; *bat_attr; ++bat_attr)
		sysfs_remove_file(*hardif_obj, &((*bat_attr)->attr));
	printk(KERN_ALERT "[PhuTran] Go to batadv_sysfs_add_hardif (3nd)\n");
out:
	printk(KERN_ALERT "[PhuTran] Go to batadv_sysfs_add_hardif (3nd)\n");
	return -ENOMEM;
}
...
{
  printk(KERN_ALERT "[PhuTran] Go to batadv_throw_uevent..!\n");
	int ret = -ENOMEM;
